...
MENU_ITEMS = [
    ...
    ("X", "Multi-Control       assign different actions to accounts  run simultaneously"),
    ("0", "Exit"),
]
...
def _prompt_action_params(action: str, cfg: dict) -> dict | None:
    """
    Prompt user for the parameters needed for a given action key.
    Returns a params dict, or None if the user cancels.
    """
    ...
    if action in ("like_user", "comment_user", "follow_user",
                  "watch_stories", "send_dm"):
...
def menu_multi_control(manager, cfg):
    """Multi-Account Simultaneous Action Controller."""
    plan: list = []   # list of {username, action, params}
    ...
    while True:
        ...
        # Always show current plan at top
        _print_plan(plan)
        rule()
        ...
        # ── 1. Add single job ─────────────────────────────────────────────────
            ...
            usernames = [b.username for b in active_bots]
            username  = Prompt.ask("  Account", choices=usernames)
            console.print()
            ...
            action_choice = Prompt.ask(
                "  Action key",
                choices=_mc.ACTION_KEYS,
            )
            params = _prompt_action_params(action_choice, cfg)
            if params is None:
                warn("Cancelled"); continue

            plan.append({"username": username, "action": action_choice, "params": params})
            ok(f"Job added: @{username} → {_mc.ACTIONS[action_choice]}")

        # ── 2. Add same job to multiple accounts ──────────────────────────────
        elif choice == "2":
            ...
            action_choice = Prompt.ask("  Action key", choices=_mc.ACTION_KEYS)
            params = _prompt_action_params(action_choice, cfg)
            if params is None:
                warn("Cancelled"); continue
            ...
            ok(f"Added {len(targets)} job(s): {action_choice}")
        ...
        # ── 5. Run plan ───────────────────────────────────────────────────────
            ...
            bots_by_username = {b.username: b for b in active_bots}
            info(f"Launching {len(plan)} job(s) across {len(set(j['username'] for j in plan))} account(s)...")
            _mc.run_plan(bots_by_username, plan, console)
        ...
        # ── 7. Load preset ────────────────────────────────────────────────────
            ...
            # Warn about accounts that aren't loaded in current session
            loaded_usernames = {b.username for b in manager.bots}
            missing = [j["username"] for j in loaded_jobs if j["username"] not in loaded_usernames]
            if missing:
                warn(f"These accounts from the preset aren't in this session: {', '.join(set(missing))}")
